   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      if (uid > 0) {
         ...
         try (Connection conn = getDBConnection()) {
            ...
            long maxSize = Integer.parseInt(man.getProperties().getProperty("maxUploadSize")); //200 megs
            ...
            while (i.hasNext()) {
               ...
               if (fileItem.isFormField() == false) {
                  if (fileItem.getSize() > 0 && fileItem.getSize() < maxSize && fileItem.getName().toLowerCase().endsWith("zip")) {
                     File f = new File(man.getProperties().getProperty("uploadLocation") + "/" + thisUser.getLname() + thisUser.getUID() + ".zip");
                     fileItem.write(f);
