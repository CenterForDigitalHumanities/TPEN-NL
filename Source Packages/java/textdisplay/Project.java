...
//      addUserTool(conn, "Enigma", "http://ciham-digital.huma-num.fr/enigma/");
   ...
   /**
    * Copy the buttons from Project p to this Project. Ensure any existing buttons for this project are removed before bringing buttons over.
    * p.projectID is the project you are copying from
    * projectID is the resulting project from the copy
    */
   public synchronized void copyButtonsFromProject(Connection conn, Project p) throws SQLException {
      try (PreparedStatement table = conn.prepareStatement("CREATE TEMPORARY TABLE tmpbuttons LIKE projectbuttons")) {
         ...
         try (PreparedStatement del = conn.prepareStatement("DELETE FROM projectbuttons WHERE project=?")) {
            System.out.println("Delete all buttons (XML) of "+projectID);
            ...
            try (PreparedStatement ins = conn.prepareStatement("INSERT INTO tmpbuttons (SELECT * FROM projectbuttons where project=?)")) {
               System.out.println("Bring over buttons (XML) from "+p.projectID);
               ...
               try (PreparedStatement upd = conn.prepareStatement("UPDATE tmpbuttons SET project=?")) {
                   System.out.println("Set projectID for tmpbuttons to be "+projectID);
   ...
   /**
    * Copy the character hotkeys from another project. Ensure any existing characters for this project are removed before bringing characters over.
    * p.projectID is the project you are copying from
    * projectID is the resulting project from the copy
    */
