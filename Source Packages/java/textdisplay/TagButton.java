...
//            b = new TagButton(uid, 4, "temp", "button description");
   ...
   public static String getAllProjectButtons(int projectID) throws SQLException {
      ...
      try {
         ...
         /* Buttons that should be available for ALL PROJECTS go here */
//         jo.element("position", 1);
//         jo.element("tag", "n");
//         jo.element("description","n");
//         ja.add(jo);
//         jo.element("position", 2);
//         jo.element("tag", "m");
//         jo.element("description","m");
//         ja.add(jo);
//         jo.element("position", 3);
//         jo.element("tag", "r");
//         jo.element("description","r");
//         ja.add(jo);
//         jo.element("position", 4);
//         jo.element("tag", "er");
//         jo.element("description","er");
//         ja.add(jo);
...
//            b = new TagButton(projectID, 4, "temp", true, "button description");
   ...
   public void deleteTag() throws SQLException {
      ...
      try {
         ...
         ps.execute();
         //now reorder them
         //THis assumes the position is perfectly indexed, which is not guaranteed.
...
//         }
    ...
    * @throws SQLException
    */
   public void setTag(String newTag, String newDesc) throws SQLException {
      Connection j = null;
      PreparedStatement stmt = null;
      try {
         String query = "update buttons set text=?,description=? where uid=? and position=?";
         if (projectID > 0) {
            query = "update projectbuttons set text=?, description=? where project=? and position=?";
         }
         j = DatabaseWrapper.getConnection();
         stmt = j.prepareStatement(query);
         stmt.setString(1, newTag);
